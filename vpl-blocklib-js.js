	...
	(function () {
		...
		var libPatchJS = {
			...
			"ground": function (block) {
				...
				if (cond === "") {
					for (var i = 1; i < 2; i++) {
						cond += " || this.get(\"prox.ground.delta\")[" + i + "] >= 450";
					}
					cond = cond.slice(4);	// crop initial " || "
				}
				return {
			...
			"motor": function (block) {
				return {
					...
					statement:
						"this.set(\"motor.left\", " + block.param[0].toFixed(2) + ");\n" +
						"this.set(\"motor.right\", " + block.param[1].toFixed(2) + ");\n"
				};
			},
			"move": function (block) {
				/** @const */
			...
			"top color": function (block) {
				return {
					...
					statement:
						"this.set(\"leds.top\", [" +
						block.param.map(function (x) { return x.toFixed(2); }).join(", ") +
						"]);\n"
				};
			},
