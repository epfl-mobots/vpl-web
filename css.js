...
/** Convert a length (number+unit) to a length in px (number)
	@param {string} length
	@param {number=} baseLength length used with percentage (default: % forbidden)
	@return {number}
*/
CSSParser.prototype.convertLength = function (length, baseLength) {
	...
	if (re == null) {
		...
		if (baseLength == undefined) {
			throw "Illegal use of percentage in length";
		}
		sc = baseLength;
